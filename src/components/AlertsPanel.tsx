...
export function AlertsPanel({ onRoundaboutSelect, alerts }: AlertsPanelProps) {
  ...
  const groupedAlerts = sortedAlerts.reduce((acc, alert) => {
    if (!acc[alert.districtName]) acc[alert.districtName] = [];
    ...
    >
            ...
            <div className="bg-slate-50 px-6 py-2 sticky top-0 border-b border-slate-200">
              <div className="flex items-center justify-between">
                ...
                <span className="text-slate-500 text-xs">
                  {
                    districtAlerts.filter(a => !a.acknowledged && !acknowledgedAlerts.has(a.id)).length
                  } فعال
                </span>
              </div>
            </div>
      ...
      <div className="p-4 border-t border-slate-200 bg-slate-50 flex-shrink-0">
        <div className="flex items-center justify-between text-sm">
          ...
          <button className="text-blue-600 hover:text-blue-700">عرض الكل</button>
        </div>
      </div>
      ...
      {dispatchAlertId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" dir="rtl">
          <div className="bg-white w-full max-w-md rounded-xl shadow-lg p-6 border border-slate-200">
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-4">إرسال فريق للموقع</h3>
            ...
            <form
              onSubmit={(e) => {
                ...
                console.log({
                  alertId: dispatchAlertId,
                  team: data.get('team'),
                  notes: data.get('notes'),
                  eta: data.get('eta'),
                });

                alert('تم إرسال الفريق!');
                setDispatchAlertId(null);
              }}
              className="space-y-4"
            >
              <div>
                <label className="block text-sm mb-1">الوحدة المباشرة</label>
                <select name="team" className="w-full border rounded-md p-2" required>
                  <option value="">--</option>
                  <option value="traffic"> الإسعاف</option>
                  <option value="maintenance"> نجم</option>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="مثال: ازدحام في المسار الجنوبي"
                ></textarea>
              </div>

              <div>
                <label className="block text-sm mb-1">الوقت المتوقع للوصول (دقائق)</label>
                <input
                  type="number"
                  name="eta"
                  className="w-full border rounded-md p-2"
